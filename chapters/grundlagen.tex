...
Eine Herausforderung beim Klassifizieren mit neuronalen Netzen ist es, einen Klassifikator zu trainieren, der auch mit bisher unbekannten Daten gute Ergebnisse erzielt \cite{srivastava2014dropout}. Man spricht auch von der Generalisierbarkeit des Klassifikators. Wenn ein Modell nur mit den Trainingsdaten gute Ergebnisse erzielen kann, spricht man von Überanpassung (engl. Overfitting). Wenn im Gegensatz dazu ein Modell schon mit den Trainingsdaten sehr schlechte Klassifizierungsergebnisse erzielt, spricht man von Unteranpassung (engl. Underfitting). In Abbildung \ref{fig_overfitting} ist beispielhaft eine Unter- und Überanpassung eines binären Klassifikators dargestellt. \todo{relevant? raus oder erweitern}
...
\noindent Diese Funktionen sind, zusammen mit der zuvor vorgestellten Treppenfunktion, in Abbildung \ref{fig_aktivierungsfunktionen} dargestellt.
...
\begin{equation}
o_k = p(c_k|x) = \frac{e^{x^\top w}}{\sum_{j=1}^C{e^{x_j^\top w}}}
\end{equation}
